...
if(isset($_GET['action']) && $_GET['action'] == "show_all_emp"){
  // $id = $_SESSION['user']['id'];
...
if (isset($_POST['action']) && $_POST['action'] == "update_emp") {
  ...
  $sql = "UPDATE `employee` SET `name`='$name',`designation`='$designation',`salary`='$salary',`joining`='$join',`password`='$pass'  where ID = '$id' ";
   //echo $sql;
  $res = mysqli_query($db,$sql);
